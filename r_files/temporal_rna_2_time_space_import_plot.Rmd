...
Do we find the expected genes?

```{r}
## p1 vs p2
genes_p1_p2_anydays<- top_domain_comparisons %>% filter(grepl("_Gate_p2_vs_p1", Comparison))

genes_p1_p2_Ncomparisons <- genes_p1_p2_anydays %>% group_by(GeneID) %>% summarise(Ncomparisons=n())
genes_p1_p2_Ncomparisons %>% arrange(desc(Ncomparisons)) %>% head(20)


## pMN vs p1

genes_pM_p1_anydays<- top_domain_comparisons %>% filter(grepl("_Gate_pM_vs_p1", Comparison))

genes_pM_p1_Ncomparisons <- genes_pM_p1_anydays %>% group_by(GeneID) %>% summarise(Ncomparisons=n())
genes_pM_p1_Ncomparisons %>% arrange(desc(Ncomparisons)) %>% head(20)


## pMN vs p2
genes_pM_p2_anydays<- top_domain_comparisons %>% filter(grepl("_Gate_pM_vs_p2", Comparison))

genes_pM_p2_Ncomparisons <- genes_pM_p1_anydays %>% group_by(GeneID) %>% summarise(Ncomparisons=n())
genes_pM_p2_Ncomparisons %>% arrange(desc(Ncomparisons)) %>% head(20)
...
### Plot the genes in heatmap to visualize trends?
...
write.csv(top_days_comparisons,
          paste0(workingdir,subworkinput,"Top_days_genes_vsd.csv"),
          quote = FALSE)
...
ggplot(go_enrichment_plot %>% filter(ONTOLOGY=="BP"), aes(x=GeneRatio, y=Description)) +
...
#### Different option: functional group first

This just highlights clusters 2 and 3

```{r}
# go_enrichment_allclusterBP <- lapply(clusterlist, function(x){
#   df = as.data.frame(out) %>% filter(Cluster==x)
#   
#   #get the most complete gene list without NAs
#   gene_list <- bitr(df$GeneID, fromType = "ALIAS",
...
# 
# 
# simplifyGOFromMultipleLists(go_enrichment_allclusterBP, go_id_column="ID",padj_column="p.adjust", ont="BP")
